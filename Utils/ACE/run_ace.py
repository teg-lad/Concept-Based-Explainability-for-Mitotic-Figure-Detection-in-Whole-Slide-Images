...
def main():
    # Specify the target class and the source directory. Note: The target class should be a folder in the source
    # directory. This folder should have a folder called discovery for images for concept discovery and a folder
    # called tcav for tcav score calculation.
    target_class = "mitotic figure"
    source_dir = Path("D:/DS/DS4/Project/Mitotic_figures")

    # Create a list of the bottleneck layers.
    bottleneck_layers = ['backbone.body.layer1.2.conv1', 'backbone.body.layer2.3.conv1', 'backbone.body.layer3.5.conv1',
    ...
    tissue_types = ['canine cutaneous mast cell tumor', 'canine lung cancer', 'canine lymphoma', 'human breast cancer',
    ...
    for tissue_type in tissue_types:
        try:
            ...
            # Once this is complete (patches_created is a boolean stating if the process ran) we will create patches
            # from the context images. The motivation for this is that we will get segments that relate to the
            # context around the annotation, which can be used to classify mitotic figures. This context may capture
            # how close neighbouring cells are, which can sometimes be used to identify mitotic figures.
            if patches_created:
            ...
            #     if bn in ['backbone.body.layer2.3.conv1', 'backbone.body.layer3.5.conv1'] and tissue_type not in [
            ...
            # Randomly select image segments to create a random concept and random groups to train the concepts
            # against. The random concept will allow us to test if our discovered concepts are statistically
            # different from a random sample. The random groups (25) will allow us to generate 25 different vectors
            # for each concept against the random group.
            print("Initializing random concepts")
            cd.initialize_random_concept_and_samples()

            # Now we calculate the CAVs by creating a linear classifier that splits the concept activations from the
            # random activations. The vector orthogonal to the decision boundary gives us the vector that represents
            # the concept.
            print("Computing CAVs")
            cav_accuracies = cd.cavs()

            # Now we compute the TCAV scores by getting the product of our gradients by the CAV and assessing how
            # many of the predictions it had a positive impact on.
            print("Computing TCAV scores")
            scores = cd.tcavs(test=False, sort=False)

            # Generate a textual report that contains how well the CAVs were split from the random, the average TCAV
            # score of the concepts and a p-value for a 2 sided t-test to see if the concepts tcav scores were
            # statistically different from the random concept's scores.
            print("Generating report")
            cd.save_ace_report()

            # Plot examples of discovered concepts and also the concepts that were influential and statistically
            # different from the random concept.
